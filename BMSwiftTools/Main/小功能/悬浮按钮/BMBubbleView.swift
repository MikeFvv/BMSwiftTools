...
//  venom
//
//  Created by BLOM on 9/29/22.
//  Copyright © 2022 Venom. All rights reserved.
//

import UIKit

private let BMKeyWindow = UIApplication.shared.windows[0]

public enum BMBubblePosition {
    case Left
    case Right
}

public class BMBubbleView: UIView {
    
    private var contentView : UIView?
    private var marginInsets = UIEdgeInsets.zero
    private var tapBlock : (()->Void)?
    
    public var position = BMBubblePosition.Left {
        didSet{
            updatePosition()
        }
    }
...
public class BMBubbleWindow: UIWindow {
    ...
    private func locationChange(point:CGPoint) {
        ...
        newCenter.y = min(BMKeyWindow.bounds.size.height-bounds.size.height*0.5-curentAreaInsets.bottom, newCenter.y)
        newCenter.x = max(bounds.size.width*0.5+curentAreaInsets.left, newCenter.x)
        newCenter.x = min(BMKeyWindow.bounds.size.width-bounds.size.width*0.5-curentAreaInsets.right, newCenter.x)
        center = newCenter;
//        print("\n手指位置--->\(point) \n气泡中心--->\(center) \n当前安全偏移--->\(curentAreaInsets) \nKeyWindow--->\(BMKeyWindow.bounds) \n气泡大小--->\(bounds) \n结果--->\(newCenter)")
    }
    
    private func locationChangeEnded() {
        var curentCenter = center
        if curentCenter.x > BMKeyWindow.bounds.size.width*0.5 {
            curentCenter.x = BMKeyWindow.bounds.size.width-bounds.size.width*0.5-curentAreaInsets.right
        }else{
    ...
    private func updatePosition() {
        ...
        let superViewHeight = BMKeyWindow.bounds.height
    ...
    private var curentAreaInsets : UIEdgeInsets{
        ...
        if #available(iOS 11.0, *) {
            insets = BMKeyWindow.safeAreaInsets
        }
    ...
    private func isiPhoneXScreen() -> Bool {
        ...
        return BMKeyWindow.safeAreaInsets != UIEdgeInsets.zero
    }
}
