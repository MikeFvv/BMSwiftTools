...
/*
func bmRegex() {
    var str = "225Hello, playground 9527 8abc 9哟吼吼 12哟吼吼"

    // "^[0-9]+$"匹配一个只能是0-9的数字组成的字符串，'^'表示以以'^'字符后面的条件为起始，'$'表示以以'$'字符前面的条件结束，
    // '+'表示'+'前面的条件有一个或者多个
    // '[]'中括号括起来标示一个独立条件
    // 例如换成"[0-9]+$"则是匹配以数字结尾的字符串，"^[0-9]+"匹配以数字开头的字符串，"[0-9]+"匹配字符串中的所有数字并存储，
    // "[0-9]" 匹配每一个数字字符并存储
    //let numPattern = "^[0-9]+$"
    let numPattern = "[0-9]+"

    do {
        // 尝试创建正则表达式对象，如果pattern不符合正则表达式语法，则会抛出异常
        let regex = try NSRegularExpression(pattern: numPattern, options: NSRegularExpression.Options.caseInsensitive)

        // 返回匹配到的结果，[NSTextCheckingResult] 如果没有匹配到内容，则返回空数组
        let result = regex.matches(in: str, options: NSRegularExpression.MatchingOptions.reportProgress, range: NSMakeRange(0, str.characters.count))

        // 替换掉符合规则的内容
        regex.stringByReplacingMatches(in: str, options: [], range: NSMakeRange(0, str.length), withTemplate: "ABC")

        // 匹配到几个结果，结果 = result.count
        regex.numberOfMatches(in: str, options: NSRegularExpression.MatchingOptions.reportProgress, range: NSMakeRange(0, str.characters.count))

        result.map {
            (str as NSString).substring(with: $0.range)
        }
    } catch {
